...
from webauth.models import User, Tool, UserAccess

logger = logging.getLogger(__name__)

def validate_tool(tool):
    '''
    Return True or False depending if the tool is supported.
    '''
    ...
    if validate_tool(tool):
        ...
        if request.method == 'POST':
            if authenticate(request.POST.get('username'), request.POST.get('password')):
                ...
                tool_obj = Tool.objects.get(short_name=tool)
                logger.info('user:%s', user_obj.id)
                logger.info('access:%s', UserAccess.objects.get(user_id=user_obj.id, tool_id=tool_obj.id))
